...
class HotelBillRecord:
    def __init__(self,window):
        self.window=window
        self.window.title("HotelBillRecord")
        ...
        Headinglabel=Label(self.window,text="HotelBillRecord",font=("times new roman",19,"bold"),bg="black",fg="gold",bd=4,relief=RIDGE,width=85)
        Headinglabel.place(x=0,y=0)

        RefreshButton=Button(self.window,text="Refresh",font=("times new roman",16,"bold"),bg="black",fg="gold",bd=2,relief=RIDGE,width=16,command=self.HotelBillRecord)
        ...
        #function calling
        self.HotelBillRecord()

    def HotelBillRecord(self,querys="select * from hotelbillrecord "):
        tree=ttk.Treeview(self.window)
        tree["columns"]=("serial","Bill","Amount","Datetime")
        ...
        tree.column("serial",width=70,minwidth=70,anchor=tk.CENTER)
        ...
        tree.heading("serial",text="Sr_No.",anchor=tk.CENTER)
    ...
    def Search(self):
        ...
        if mode=="datetime":
            value=value+"%"
            print(value)
        querys="select * from hotelbillrecord where {} LIKE '{}'".format(mode,value)
        my_cursor.execute(querys)
        for i in my_cursor:
            print(i)
        self.HotelBillRecord(querys)
