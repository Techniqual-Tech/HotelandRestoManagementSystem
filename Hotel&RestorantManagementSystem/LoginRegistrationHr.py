...
class LoginRegistration:
    def __init__(self,window):
        ...
        heading=Label(self.window,text="LoginRegistration",font=("times new roman",25,"bold"),bg="black",fg="gold",bd=4,relief=RIDGE,width=65)
        heading.place(x=0,y=0)

        self.mainframe=Frame(self.window,bg="grey",width="1280",height="700",relief=RIDGE,bd=4)
        self.mainframe.place(x=0,y=48)
                                    ...
                                    if upper==a or lower==a:
                                        ...
                                        if  self.Passwordfield.get()== self.CPasswordfield.get():
                                            try:
                                                ...
                                                query="select COUNT(*) from loginregisterhr WHERE Username='{}'".format(self.Usernamefield.get())
                                                my_cursor.execute(query)
                                                for i in my_cursor:
                                                    print(i[0])
                                                if i[0]==0:
                                                    my_cursor.execute("insert into loginregisterhr values(%s,%s,%s,%s,%s,%s)",(
                                                                                                               self.Namefield.get(),
    ...
    def Unsubscribed(self):
        ...
        conn=mysql.connector.connect(host="localhost",username="root",password="vicky@123//",database="hotelandrestoproject")
        my_cursor=conn.cursor()
        query="select COUNT(*) from loginregisterhr WHERE Username='{}' AND Password='{}' AND BestFriend='{}'".format(self.DUsernamefield.get(),self.DPasswordfield.get(),self.DBestFfield.get())
        my_cursor.execute(query)
        for i in my_cursor:
            print(i[0])
        if self.DUsernamefield.get()=="" or self.DPasswordfield.get()=="" or self.DBestFfield.get()=="":
            ...
            if i[0]==0:
                ...
                queryd="DELETE from loginregisterhr WHERE Username='{}' AND Password='{}' AND BestFriend='{}'".format(self.DUsernamefield.get(),self.DPasswordfield.get(),self.DBestFfield.get())
